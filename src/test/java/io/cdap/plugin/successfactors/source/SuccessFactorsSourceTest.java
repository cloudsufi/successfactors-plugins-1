...
public class SuccessFactorsSourceTest {
  ...
  @Test
  public void testConfigurePipelineWithInvalidAdvanceParam() {
    pluginConfigBuilder = SuccessFactorsPluginConfig.builder()
      .referenceName("unit-test-ref-name")
      .baseURL("http://localhost")
      .entityName("entity name")
    ...
    try {
      ...
      MockPipelineConfigurer mockPipelineConfigurer = new MockPipelineConfigurer(null, plugins);
      successFactorsSource.configurePipeline(mockPipelineConfigurer);
      Assert.fail("Exception will not thrown if valid values will be provided");
    } catch (ValidationException ve) {
      List<ValidationFailure> failures = ve.getFailures();
      ...
      Assert.assertEquals(ResourceConstants.ERR_NEGATIVE_PARAM_PREFIX
                            .getMsgForKey("Number of Rows to Fetch"), failures.get(1).getMessage());

      Assert.assertEquals(ResourceConstants.ERR_NEGATIVE_PARAM_PREFIX
                            .getMsgForKey("Batch Size"), failures.get(2).getMessage());
    }
  }
    ...
    new Expectations(SuccessFactorsService.class, SuccessFactorsTransporter.class) {
      {
        ...
        successFactorsPartitionBuilder.buildSplit(50, 1000L,
                                                  pluginConfig.getSkipRowCount(), pluginConfig.getSplitCount(),
