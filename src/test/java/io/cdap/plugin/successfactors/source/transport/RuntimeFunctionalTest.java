...
public class RuntimeFunctionalTest {
  ...
  public void runPipelineWithDefaultValues() throws Exception {
    ...
    List<SuccessFactorsInputSplit> partitionList = new SuccessFactorsPartitionBuilder().buildSplit(availableRowCount,
                                                                                                   pluginConfig.
                                                                                                     getNumRowsToFetch()
      , pluginConfig.getSkipRowCount(), pluginConfig.getSplitCount(), pluginConfig.getBatchSize());
  ...
  public void verifyTotalRecordFetchAndNetworkCalls() throws IOException, InterruptedException,
    ...
    for (SuccessFactorsInputSplit inputSplit : partitionList) {
      ...
      Assert.assertEquals(msg, expectedRecordsToPull, recordList.size());
      int expectedNetworkCallCount = (int) (expectedRecordsToPull / inputSplit.getPackageSize());
      verify(expectedNetworkCallCount, getRequestedFor(WireMock.urlPathEqualTo("/odata/v2/Background_" +
                                                                                 "SpecialAssign/$metadata")));
    }
  }
